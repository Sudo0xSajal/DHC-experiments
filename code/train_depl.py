...
from utils import maybe_mkdir, get_lr, fetch_data, seed_worker, poly_lr, xavier_normal_init_weight, kaiming_normal_init_weight
...
def initial_qhat(class_num=14):
    # initialize qhat of predictions (probability)
    qhat = torch.ones((1, class_num)+config.patch_size, dtype=torch.float) / class_num
    print("qhat size: ", qhat.size())
    return qhat.cuda()
...
if __name__ == '__main__':
    ...
    best_eval = 0.0
    best_epoch = 0
    qhat_A = initial_qhat(class_num=config.num_cls)
    qhat_B = initial_qhat(class_num=config.num_cls)
    tau = 1.0
        ...
        for batch_l, batch_u in tqdm(zip(labeled_loader, unlabeled_loader)):
            ...
            if args.mixed_precision:
                with autocast():
                    ...
                    pseudo_label_A = causal_inference(output_A.detach(), qhat_A, tau=tau)
                    pseudo_label_B = causal_inference(output_B.detach(), qhat_B, tau=tau)

                    max_probs_A, pseudo_targets_A = torch.max(pseudo_label_A, dim=1)
                    mask_A = max_probs_A.ge(0.9).float()
                    max_probs_B, pseudo_targets_B = torch.max(pseudo_label_B, dim=1)
                    mask_B = max_probs_B.ge(0.9).float()

                    # update qhat
                    qhat_A = update_qhat(torch.softmax(output_A_u.detach(), dim=1), qhat_A, momentum=0.99)
                    qhat_B = update_qhat(torch.softmax(output_B_u.detach(), dim=1), qhat_B, momentum=0.99)

                    # adaptive marginal loss
                    delta_logits_B = torch.log(qhat_B)
                    output_A_u = output_A_u + tau * delta_logits_B

                    delta_logits_A = torch.log(qhat_A)
                    output_B_u = output_B_u + tau * delta_logits_A

                    # pseudo_targets_A = torch.argmax(pseudo_label_A, dim=1, keepdim=True).long()
                    # pseudo_targets_B = torch.argmax(pseudo_label_B, dim=1, keepdim=True).long()


                    loss_cps = cps_loss_func(output_A, pseudo_targets_B, mask_B) + cps_loss_func(output_B, pseudo_targets_A, mask_A)
                    # loss prop
